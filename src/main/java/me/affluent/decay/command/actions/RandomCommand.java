...
public class RandomCommand extends BotCommand {
    ...
    protected void execute(CommandEvent e) {
        ...
        User target = getRandomTarget(uid, p.getRank(), 0);
        if (target == null) {
            e.reply(MessageUtil.err(Constants.ERROR(uid), Language.getLocalized(uid, "random_pvp_no_target",
                    "This land appears to be empty, maybe try searching elsewhere.\nPlease try again (later).")));
            return;
        }
        ...
        if (tw == null) {
            String msg1 = Language.getLocalized(uid, "target_no_weapon", "Aha, that fool! They hath no weapon, quick, kill them!");
            e.reply(MessageUtil.err(Constants.ERROR(uid), msg1));
            return;
        }
        if (CooldownUtil.hasCooldown(uid, "pvp_barrier")) CooldownUtil.removeCooldown(uid, "pvp_barrier");
        Fight fight = new Fight(p, t, pw, tw);
        Player winner = fight.doFight(false, true);
        ...
        /*int randomChest = new Random().nextInt(101);
        if (randomChest <= 1) {
            if (args.equals("claim")) {
    ...
    private User getRandomTarget(String userId, Rank rank, int rerollTime) {
        int power = Power.getFullPower(Player.getPlayer(userId));
        int minPower = (int) (power * 0.75);
        int maxPower = (int) (power * 1.3);
        //int minLevel = rank.getRequiredLevel();
        //int maxLevel = rank.getMaxLevel();
        int randomCount = 0;
        /*try (ResultSet rs = Withering.getBot().getDatabase()
                .query("SELECT COUNT(*) FROM profiles LEFT JOIN exp ON exp.userId = profiles.userId LEFT JOIN health " +
                       "ON health.userId = profiles.userId WHERE health.health > 0 AND exp.level >= " + minLevel +
                       " AND exp.level <= " + maxLevel + " AND profiles.userId != '" + userId + "';")) {
            if (rs.next()) randomCount = rs.getInt("COUNT(*)");*/
        try (ResultSet rs = Withering.getBot().getDatabase()
                .query("SELECT COUNT(*) FROM profiles LEFT JOIN power ON power.userId = profiles.userId LEFT JOIN health " +
                        "ON health.userId = profiles.userId WHERE health.health > 0 AND power.totalPower >= " + minPower +
                        " AND power.totalPower <= " + maxPower + " AND profiles.userId != '" + userId + "';")) {
            if (rs.next()) randomCount = rs.getInt("COUNT(*)");
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        if (randomCount == 0) return null;
        String randomUserId = null;
        int randomChoose = new Random().nextInt(randomCount);
        /*try (ResultSet rs = Withering.getBot().getDatabase()
                .query("SELECT profiles.userId FROM profiles LEFT JOIN exp ON exp.userId = profiles.userId LEFT JOIN " +
                       "health ON health.userId = profiles.userId WHERE health.health > 0 AND exp.level >= " +
                       minLevel + " AND exp.level <= " + maxLevel + " AND profiles.userId != '" + userId + "' LIMIT " +
                       randomChoose + ", 1;")) {*/
        try (ResultSet rs = Withering.getBot().getDatabase()
                .query("SELECT profiles.userId FROM profiles LEFT JOIN power ON power.userId = profiles.userId LEFT JOIN " +
                        "health ON health.userId = profiles.userId WHERE health.health > 0 AND power.totalPower >= " +
                        minPower + " AND power.totalPower <= " + maxPower + " AND profiles.userId != '" + userId + "' LIMIT " +
                        randomChoose + ", 1;")) {
            if (rs.next()) {
                randomUserId = rs.getString("userId");
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        if (randomUserId == null) return null;
        if (CooldownUtil.hasCooldown(randomUserId, "pvp_barrier")) {
            if (rerollTime == 9) return null;
            return getRandomTarget(userId, rank, rerollTime + 1);
        }
        return Player.getUser(randomUserId);
    }
